...
public class TaskServiceCreateTaskTest {
    ...
    void shouldCreateTaskAndReturnDtoWhenTokenAndTaskRequestAreValid(){
        ...
        TaskResponseDTO result = taskService.createUserTask(VALID_TOKEN, taskRequest);
    ...
    void shouldThrowJwtExceptionWhenTokenIsInvalid() {
        ...
        assertThatThrownBy(() -> taskService.createUserTask(invalidToken, taskRequest))
                .isInstanceOf(JwtException.class)
                .hasMessage("Invalid or expired JWT token.");

        ArgumentCaptor<Task> taskCaptor = ArgumentCaptor.forClass(Task.class);
        verify(taskRepository, times(0)).save(taskCaptor.capture());

        verify(jwtUtil, times(1)).extractUserId(invalidToken);
        verify(mapper, times(0)).toEntity(taskRequest);
        verify(entityManager, times(0)).getReference(User.class, userId);
